...
class TestGovernanceRules(unittest.TestCase):
    ...
    def test_governance_rules_are_built_from_contract(self) -> None:
        manifest = load_contract(self.root, "governance")
        rules = build_governance_rules(manifest)
        self.assertGreater(len(rules), 0)
        self.assertTrue(all(rule.domain == "governance" for rule in rules))

    def test_governance_rule_evaluation_returns_expected_shape(self) -> None:
        manifest = load_contract(self.root, "governance")
        rules = build_governance_rules(manifest)
