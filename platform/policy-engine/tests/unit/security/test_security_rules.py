...
class TestSecurityRules(unittest.TestCase):
    ...
    def test_security_rules_are_built_from_contract(self) -> None:
        manifest = load_contract(self.root, "security")
        rules = build_security_rules(manifest)
        self.assertGreater(len(rules), 0)
        self.assertTrue(all(rule.domain == "security" for rule in rules))

    def test_security_rule_evaluation_returns_expected_shape(self) -> None:
        manifest = load_contract(self.root, "security")
        rules = build_security_rules(manifest)
