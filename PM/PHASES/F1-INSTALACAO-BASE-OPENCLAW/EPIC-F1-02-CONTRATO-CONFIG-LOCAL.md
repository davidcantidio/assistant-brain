...
- `bash scripts/verify_linux.sh` executado com sucesso.
...
**Plano TDD**
1. `Red`: executar `bash scripts/verify_linux.sh` com variavel obrigatoria ausente e observar falha.
2. `Green`: preencher variaveis obrigatorias conforme `DEV/DEV-OPENCLAW-SETUP.md`.
3. `Refactor`: rerodar `verify_linux.sh` para validar consistencia do contrato.

**Criterios de aceitacao**
- Given variavel obrigatoria ausente, When `verify_linux.sh` roda, Then o check falha com indicacao de requisito faltante.
- Given variaveis obrigatorias preenchidas, When `verify_linux.sh` roda, Then o check de ambiente nao falha por ausencia de configuracao.
...
**Plano TDD**
1. `Red`: validar divergencia proposital em default critico (`HEARTBEAT_MINUTES`, base URL de gateway ou alias de supervisor).
2. `Green`: ajustar defaults para o contrato normativo.
3. `Refactor`: executar `make eval-models` para confirmar alinhamento.

**Criterios de aceitacao**
- Given defaults divergentes, When checks de modelo/roteamento sao executados, Then o baseline e reprovado.
